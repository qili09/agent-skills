...
| [time-report](skills/time-report/) | Log the time spent on a Jira story during a Claude Code session as a Tempo worklog (via the Tempo REST API so the Billing Key attribute is set; Jira API as warned fallback), computed from the session transcript: active time between messages rounded to 15 min, the story taken from user-authored text only, one-line confirmation before posting, a local ledger to prevent double-logging, and a Stop hook that nudges whenever a story has unlogged time (15-min floor). |
